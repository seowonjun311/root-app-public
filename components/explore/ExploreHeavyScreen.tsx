...
const EXPLORATION_BACKGROUND_SYNC_DELAY_MS =
  24 * 60 * 60 * 1_000;

const EXPLORATION_BACKGROUND_SYNC_INTERVAL_MS =
  5 * 60 * 1_000;

let explorationBackgroundSyncPromise:
  ReturnType<typeof syncExplorationData> | null =
    null;

let explorationBackgroundSyncLastStartedAt =
  0;

function startExplorationBackgroundSync() {
  const now =
    Date.now();

  if (
    explorationBackgroundSyncPromise
  ) {
    return explorationBackgroundSyncPromise;
  }

  if (
    now -
      explorationBackgroundSyncLastStartedAt <
    EXPLORATION_BACKGROUND_SYNC_INTERVAL_MS
  ) {
    return null;
  }
...
const SEOUL_RESERVATION_ENTRY_OPTIONS = [
  {
    id: 'camping',
    label: '캠핑·피크닉',
    icon: '🏕️',
    description: '캠핑장과 피크닉 시설',
    pathname: '/explore/facilities/camping',
  },
  {
    id: 'sports',
    label: '체육시설',
    ...
    pathname: '/explore/facilities/sports',
  },
  {
    id: 'space',
    label: '공간대관',
    ...
    pathname: '/explore/facilities/space',
  },
  {
    id: 'education',
    label: '교육·체험',
    ...
    pathname: '/explore/facilities/education',
  },
] as const;
...
export default function ExploreHeavyScreen({
  initialMapLevel = 'korea',
  initialContentMode = 'places',
}: ExploreHeavyScreenProps) {
  const { theme, isCityBlack } = useRootTheme();
  const insets = useSafeAreaInsets();

  const districtNavigationLockRef =
    useRef(false);
    ...
    useCallback(() => {
      ...
      const loadLocal = async () => {
        try {
          ...
          console.log(
            'EXPLORE SCREEN LOCAL CLOUD LOAD ERROR',
            error
          );
        }
      };
          ...
          () => {
            backgroundDelay =
              setTimeout(
                () => {
                  const syncTask =
                    startExplorationBackgroundSync();

                  if (!syncTask) {
                    console.log(
                      'EXPLORE SCREEN BACKGROUND SYNC SKIPPED: RECENTLY SYNCED'
                    );
                  ...
                  console.log(
                    'EXPLORE SCREEN BACKGROUND SYNC START'
                  );
                  ...
                  void syncTask
                    ...
                    .catch(
                      (error) => {
                        console.log(
                          'EXPLORE SCREEN BACKGROUND SYNC ERROR',
                          error
                        );
                      }
                    );
                },
                EXPLORATION_BACKGROUND_SYNC_DELAY_MS
              );
          }
        );
...
if (mapLevel === 'seoul') {
  if (
    districtNavigationLockRef.current
  ) {
    console.log(
      'EXPLORATION DISTRICT NAVIGATION LOCKED',
      {
        districtId:
          normalizedDistrictId,
      }
  ...
  console.log(
    'EXPLORATION DISTRICT NAVIGATION START',
    {
      districtId:
        normalizedDistrictId,
    }
    ...
    <View style={styles.sectionBlock}>
      <View style={styles.sectionHeader}>
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            필요한 종류를 선택하면 해당 데이터만 불러와요.
          </Text>
        </View>
      </View>

      <View style={styles.placeList}>
        {SEOUL_RESERVATION_ENTRY_OPTIONS.map(
          (option) => (
            <Pressable
              key={option.id}
              onPress={() => {
                console.log(
                  'SEOUL FACILITY CATEGORY NAVIGATION START',
                  {
                    kind: option.id,
                    districtId: 'all',
                  }
