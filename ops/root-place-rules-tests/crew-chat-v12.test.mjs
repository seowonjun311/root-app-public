// ROOT_CREW_CHAT_V12_FIRESTORE_STORAGE_TESTS
...
const PROJECT_ID = 'demo-root-crew-chat-v12';
const firestoreRules = fs.readFileSync(
  new URL('../../firebase/firestore-crew-chat-v12.rules', import.meta.url),
  'utf8'
);
const storageRules = fs.readFileSync(
  new URL('../../firebase/storage-crew-chat-v12.rules', import.meta.url),
  'utf8'
);
...
try {
  ...
  const moderator = env.authenticatedContext('root-mod', {
    rootModerator: true,
  });
  ...
  await pass('ROOT moderator can read member report', () =>
    assertSucceeds(getDoc(reportRef(moderator, 'alice-seed_bob')))
  );

  await pass('ROOT moderator can read reported message preview', () =>
    assertSucceeds(getDoc(messageRef(moderator, 'alice-seed')))
  );

  await pass('ordinary member cannot resolve report', () =>
    assertFails(
      updateDoc(reportRef(alice, 'alice-seed_bob'), {
        status: 'dismissed',
        resolution: 'dismissed',
        reviewerId: 'alice',
        reviewedAt: '2026-08-14T03:00:00.000Z',
        updatedAt: '2026-08-14T03:00:00.000Z',
      })
    )
  );

  await pass('ROOT moderator can resolve report with immutable audit fields', () =>
    assertSucceeds(
      updateDoc(reportRef(moderator, 'alice-seed_bob'), {
        status: 'dismissed',
        resolution: 'dismissed',
        reviewerId: 'root-mod',
        reviewedAt: '2026-08-14T03:00:00.000Z',
  ...
  const pushTokenRef = (context, uid = 'bob') =>
    doc(context.firestore(), 'users', uid, 'pushTokens', 'device-a');

  await pass('user can register own Expo push token', () =>
    assertSucceeds(setDoc(pushTokenRef(bob), validPushToken))
  );

  await pass('outsider cannot read another user push token', () =>
    assertFails(getDoc(pushTokenRef(outsider)))
  );

  await pass('user cannot register a token for another uid', () =>
    assertFails(
      setDoc(pushTokenRef(bob, 'alice'), {
        ...validPushToken,
        userId: 'alice',
      })
    )
  ...
  await pass('malformed push token is rejected', () =>
    assertFails(
      setDoc(doc(bob.firestore(), 'users', 'bob', 'pushTokens', 'bad'), {
        ...validPushToken,
        token: 'https://attacker.example/token',
      })
    )
  );
  ...
  await env.withSecurityRulesDisabled(async (context) => {
    await setDoc(
      doc(context.firestore(), 'crews', 'crew-a', 'messages', 'mod-delete'),
      v2Message('mod-delete', 'bob', {
        image: {
          storagePath: 'crew-chat/crew-a/bob/mod-delete/photo.jpg',
    ...
    await uploadBytes(
      ref(context.storage(), 'crew-chat/crew-a/bob/mod-delete/photo.jpg'),
      new Uint8Array([1, 2, 3]),
      { contentType: 'image/jpeg' }
    );
  });

  await pass('ROOT moderator can delete reported message', () =>
    assertSucceeds(deleteDoc(messageRef(moderator, 'mod-delete')))
  );

  await pass('ROOT moderator can delete reported photo', () =>
    assertSucceeds(
      deleteObject(
        ref(
          moderator.storage(),
          'crew-chat/crew-a/bob/mod-delete/photo.jpg'
        )
      )
    )
  );
  ...
  console.log('PASS - ROOT CREW CHAT V1.2 moderation, push-token and media security suite');
} finally {
  await env.cleanup();
}
